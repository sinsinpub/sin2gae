...
class OAuthClient(object):
    ...
    __public__ = ('callback', 'cleanup', 'login', 'logout', 'show_key', 'get_raw', 'post_raw', 'key_name')
    ...
    def get_raw(self, api_method, http_method='GET', **extra_params):
    ...
    def show_key(self):
        return "Your key: %s" % self.handler.request.get("key")

    def callback(self, return_to='/oauth/twitter/show_key?key='):
        ...
        #logger.info({"self.token":self.token})
        ...
        self.token.put()
        self.handler.redirect(return_to + key_name)
    ...
    def get_signed_body(self, __url, __token=None, __meth='GET',**extra_params):
        ...
        #logger.info({"self.service_key": self.service_key})
        #logger.info({"key": key})

        for k in kwargs:
            if not type(kwargs[k]) is str:
                continue
        ...
        ret = urlencode(kwargs).replace('%7E%7E%7E', '~')
        return ret
