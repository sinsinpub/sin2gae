...
glogger = logging.getLogger('debug')
...
class ApiProxy(BaseProxy):
  ...
  def GET(self, params):
    ...
    if web.ctx.environ.get('QUERY_STRING', None):
      target_url += '?'+web.ctx.environ['QUERY_STRING']
    httpcon = httplib.HTTPConnection('api.twitter.com', 80)
    try:
      httpcon.request('GET', target_url, headers=headers)
      twitter_response = httpcon.getresponse()
      return BaseProxy.sendoutput(self, twitter_response)
    except Exception, inst:
      if result:
        logger.error("%s\n\n%s\n\n%s\n\n%s\n\n%s" % (target_url, str(inst), headers, web.data(), twitter_response.read()))
      else:
  ...
  def POST(self, params):
    ...
    try:
      httpcon.request('POST', target_url, headers=headers, body=web.data())
      result = httpcon.getresponse()
      return BaseProxy.sendoutput(self, result)
    except Exception, why:
      if result:
        logger.error("%s\n\n%s\n\n%s\n\n%s\n\n%s" % (target_url, str(why), headers, web.data(), result.read()))
      else:
...
class SearchProxy(BaseProxy):
  ...
  def GET(self, params):
    ...
    if web.ctx.environ.get('QUERY_STRING', None):
      target_url += '?'+web.ctx.environ['QUERY_STRING']
    httpcon = httplib.HTTPConnection('search.twitter.com', 80)
    try:
      httpcon.request('GET', target_url, headers=headers)
      twitter_response = httpcon.getresponse()
      return BaseProxy.sendoutput(self, twitter_response)
      ...
      if len(content.strip()) > 0:
        ...
        if err is not None:
          ...
          try:
            ...
            httpcon = httplib.HTTPConnection('api.twitter.com', 80)
            glogger.debug('username=' + m['username'].value)
            logger.debug(str(headers))
            logger.debug(qs)
            httpcon.request('POST', '/1/statuses/update.json', headers=headers, body=qs)
            twitter_response = httpcon.getresponse()
...
class NoFilterApiProxy(ApiProxy, Filter):
  pass

class NoFilterSearchProxy(SearchProxy, Filter):
  pass
...
urls  = (
    ...
    '/base/(.*)', 'NoFilterProxy',
    '/api/(.*)', 'NoFilterApiProxy',
    '/search/(.*)', 'NoFilterSearchProxy',
    ...
    '/text/(statuses/retweets?\.json.*)', 'JSONSingleStatusesTextOnlyProxy',
    '/text/(statuses/retweets?\.xml.*)', 'XMLSingleStatusesTextOnlyProxy',
...
#    '/text/(upicme)/(api/uploadAndPost.*)', 'JSONTwitPicProxy',
    ...
    '/text/(\w+)/(\w+)/(subscribers\.json.*)', 'JSONSingleStatusesTextOnlyProxy',
    '/text/(\w+)/(\w+)/(subscribers\.xml.*)', 'XMLSingleStatusesTextOnlyProxy',
