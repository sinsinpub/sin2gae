...
class Services_Twitter
{
    ...
    function getRandomFollowingsData()
    {
        $followings = $this->_getData('GET', $this->um_friends, 'getFriends');
        $json =& new Jsphon_Decoder();
        $followings = $json->decode($followings);
        return $followings[array_rand($followings)];
    }
     function getRandomTweet($num = 20){
        $timelineData = $this->_getData('GET', $this->sm_friends, 'getFriendsTimeline');                  
        $json =& new Jsphon_Decoder();
        $timelineData = $json->decode($timelineData);
        
        $num = rand(0,$num - 1);
        for($i=0;$i<$num;$i++){
            if($timelineData[$num]["user"]["screen_name"] !== $this->_user){
                break;
            }else{
                $num++;
                if($num >= 20){
