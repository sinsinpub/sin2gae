...
class Twitter_Bot{
	...
	function Request($url,$method = "POST",$opt = array()){
		$req = $this->TO->OAuthRequest("https://api.twitter.com/1/".$url,$method,$opt);
	...
	function Get_Followers($uid){
		$opt = array();
		$req = $this->Request('followers/ids.json?user_id='.$uid,"GET",$opt);
		if($req){
			if($req->Code != "200"){die("Error: ".$req->Code);}
			$result = str_replace(":NULL,",':"NULL",',$req->Body);
		} else {die('Get_TL(): $req is NULL');}
	...
	function Get_Friends($uid){
		$opt = array();
		$req = $this->Request('friends/ids.json?user_id='.$uid,"GET",$opt);
		if($req){
			if($req->Code != "200"){die("Error: ".$req->Code);}
			$result = str_replace(":NULL,",':"NULL",',$req->Body);
		} else {die('Get_TL(): $req is NULL');}
	...
	function Follow($uid,$flg = true){
		$result = "ok";
		//$req = $this->Request("friendships/".($flg?"create":"destroy").".xml?user_id=".$uid);
